...
interface EditNodeModalProps {
  ...
  onClose: () => void;
  node: Node | null;
  onSubmit: (data: { name: string; expires_at?: string | null; status?: string; node_info?: { os: string; arch: string; hostname: string } }) => void;
  isLoading?: boolean;
}
...
export const EditNodeModal: React.FC<EditNodeModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (expiryType === 'forever') {
      ...
      const base = node?.expires_at ? new Date(node.expires_at) : now;
      expiresAt = new Date(Math.max(base.getTime(), now.getTime()) + 24 * 60 * 60 * 1000).toISOString();
    } else if (expiryType === '1w') {
      const base = node?.expires_at ? new Date(node.expires_at) : now;
      expiresAt = new Date(Math.max(base.getTime(), now.getTime()) + 7 * 24 * 60 * 60 * 1000).toISOString();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Edit: ${node?.name || 'Node'}`}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {expiryType !== 'keep' && expiryType !== 'existing' && expiryType !== 'forever' && expiryType !== 'custom' && (
        ...
        <div className="space-y-4 pt-4 border-t border-gray-100 dark:border-gray-700">
          <h4 className="text-sm font-medium text-gray-900 dark:text-white">Device Information (Optional)</h4>
